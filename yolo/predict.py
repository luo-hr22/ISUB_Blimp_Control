    ...
    if mode == "predict":
        ...
        while True:
            ...
            try:
                ...
                r_image = yolo.detect_image(image, client_socket, crop=crop, count=count)
                r_image.show()
        ...
        fps = 0.0
        while True:
            client_socket.sendall(b';11.45,19.19;8.10;')
            t1 = time.time()
            # 读取某一帧
            ref, frame = capture.read()
            if not ref:
            ...
            # 进行检测
            frame = np.array(yolo.detect_image(frame, client_socket))
        ...
        for img_name in tqdm(img_names):
            if img_name.lower().endswith(
                ...
                r_image = yolo.detect_image(image, client_socket)
